...
static void *adapter_thread(void *data)
{
   ...
   while (!a->quitting)
   {
      ...
      int size = 0;
      int result = libusb_interrupt_transfer(a->handle, EP_IN, payload, sizeof(payload), &size, 100);
      ...
      if (memcmp(rumble, a->rumble, sizeof(rumble)) != 0)
      {
         ...
         result = libusb_interrupt_transfer(a->handle, EP_OUT, a->rumble, sizeof(a->rumble), &size, 0);
         if (result != 0) {
            fprintf(stderr, "libusb_interrupt_transfer error %d\n", result);
            a->quitting = true;
            break;
...
int main(int argc, char *argv[])
{
   ...
   int hotplug_ret = libusb_hotplug_register_callback(NULL, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED | LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT, LIBUSB_HOTPLUG_NO_FLAGS, 0x057e, 0x0337, LIBUSB_HOTPLUG_MATCH_ANY, hotplug_callback, NULL, &callback);
